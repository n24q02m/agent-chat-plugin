...
def build_parser() -> argparse.ArgumentParser:
    ...
    s.add_argument("channel", help="channel name")
    s.add_argument("--members", help="comma-separated agent names")
    s.add_argument("--topic", help="initial channel topic")
    ...
    s.add_argument("--status", default="discussion", help="message status (e.g. discussion, complete)")
    s.add_argument("--body", help="message body text")
    s.add_argument("--body-file", help="path to file containing message body")
    ...
    s.add_argument("channel", help="channel name")
    s.add_argument("--from", dest="sender", required=True, help="sender agent name")
    s.add_argument("--type", dest="event_type", choices=EVENT_TYPES, required=True, help="type of event")
    s.add_argument("--harness", required=True, help="adapter harness name")
    s.add_argument("--status", choices=STATUS_VALUES, help="status value for status events")
    s.add_argument("--detail", help="optional detail message")
    s.add_argument("--primitives", action="append", help="supported primitives for capability events")
    ...
    s.add_argument("channel", help="channel name")
    s.add_argument("--as", dest="agent", required=True, help="agent identity to read as")
    s.add_argument(
    ...
    s.add_argument("channel", help="channel name")
    s.add_argument("--as", dest="agent", required=True, help="agent identity waiting for reply")
    s.add_argument("--timeout", type=float, default=900.0, help="timeout in seconds")
    ...
    s.add_argument("channel", help="channel name")
    s.add_argument("-n", type=int, default=3, help="number of messages to peek")
    ...
    s.add_argument("--as", dest="agent", required=True, help="agent identity claiming the task")
    s.set_defaults(func=cmd_claim)

    s = sub.add_parser("lock", help="lock workspace-relative paths")
    s.add_argument("channel", help="channel name")
    s.add_argument("paths", nargs="+", help="workspace-relative paths to lock")
    s.add_argument("--as", "--from", "--owner", dest="owner", required=True, help="agent identity locking the paths")
    s.add_argument("--lease-seconds", "--lease", "--ttl", type=float, default=300.0, help="lease duration in seconds")
    s.set_defaults(func=cmd_lock)

    s = sub.add_parser("check", help="check workspace-relative paths for conflicts")
    s.add_argument("channel", help="channel name")
    s.add_argument("paths", nargs="+", help="workspace-relative paths to check")
    s.add_argument("--as", "--from", "--owner", dest="owner", help="agent identity checking the paths")
    s.set_defaults(func=cmd_check)

    s = sub.add_parser("unlock", help="release an owned path lock")
    s.add_argument("channel", help="channel name")
    s.add_argument("target", help="lock id or exact normalized path")
    s.add_argument("--as", "--from", "--owner", dest="owner", required=True, help="agent identity releasing the lock")
    s.set_defaults(func=cmd_unlock)

    s = sub.add_parser("recover", help="recover an expired path lock explicitly")
    s.add_argument("channel", help="channel name")
    s.add_argument("target", help="lock id or exact normalized path")
    s.add_argument("--as", "--from", "--owner", dest="owner", required=True, help="agent identity recovering the lock")
    s.add_argument("--reason", required=True, help="reason for recovery")
    s.add_argument("--lease-seconds", "--lease", "--ttl", type=float, default=300.0, help="new lease duration in seconds")
    ...
    s.add_argument("channel", help="channel name")
    s.add_argument("--as", "--from", "--owner", dest="actor", required=True, help="agent identity recovering the transaction")
    s.add_argument(
        "--resolve-publication",
    ...
    s.add_argument("channel", help="channel name")
    s.add_argument("task_id", help="unique task identifier")
    s.add_argument("--from", "--created-by", dest="creator", required=True, help="agent creating the task")
    s.add_argument("--title", required=True, help="task title")
    s.add_argument("--owner", help="agent assigned to the task")
    s.add_argument("--depends-on", action="append", default=[], help="task dependencies")
    s.add_argument("--files-hint", action="append", default=[], help="files relevant to the task")
    ...
    s.add_argument("--title", default=argparse.SUPPRESS, help="new task title")
    s.add_argument("--owner", default=argparse.SUPPRESS, help="new task owner")
    s.add_argument("--clear-owner", action="store_true", help="clear the current owner")
    s.add_argument("--depends-on", action="append", default=argparse.SUPPRESS, help="add task dependencies")
    s.add_argument("--files-hint", action="append", default=argparse.SUPPRESS, help="add relevant files")
    s.add_argument("--acceptance", action="append", default=argparse.SUPPRESS, help="add acceptance criteria")
    s.add_argument("--branch", default=argparse.SUPPRESS, help="new task branch")
    s.add_argument("--clear-branch", action="store_true", help="clear the current branch")
    s.add_argument("--status", default=argparse.SUPPRESS, help="new task status")
    ...
    s.add_argument("channel", help="channel name")
    s.add_argument("task_id", help="unique task identifier")
    s.add_argument("--as", "--from", dest="actor", required=True, help="agent renewing the lease")
    s.add_argument("--lease-seconds", "--lease", "--ttl", type=float, default=300.0, help="lease duration in seconds")
    ...
    s.add_argument("channel", help="channel name")
    s.add_argument("task_id", help="unique task identifier")
    s.add_argument("--as", "--from", dest="actor", required=True, help="agent recovering the lease")
    s.add_argument("--reason", required=True, help="reason for recovery")
    s.add_argument("--lease-seconds", "--lease", "--ttl", type=float, default=300.0, help="new lease duration in seconds")
    ...
    for command, handler, help_text, action in (
        ...
        s.add_argument("channel", help="channel name")
        s.add_argument("task_id", help="unique task identifier")
        s.add_argument("--as", "--from", dest="actor", required=True, help="agent acting on the task")
