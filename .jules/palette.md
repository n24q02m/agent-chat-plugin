...
## 2026-07-29 - Prevent hanging process appearance for stdin reads
**Learning:** When reading from stdin in a CLI application (`sys.stdin.read()`), interactive users may think the process has hung if there's no prompt. Adding a prompt breaks pipes if written to stdout.
**Action:** Use `sys.stdin.isatty()` to provide a helpful prompt (e.g., "(reading body from stdin, press Ctrl-D to finish)") to `sys.stderr` during interactive use, preventing the appearance of a hanging process without breaking pipes.
