...
## 2024-08-14 - CLI Argparse Help Text
**Learning:** Adding descriptive `help` text for all `argparse` arguments (both positional and optional) significantly improves usability by making the interface self-documenting via the `--help` flag, which is a key accessibility feature for CLI tools.
**Action:** Ensure all positional and optional arguments in `argparse` have explicit, descriptive `help="..."` attributes to avoid forcing users to read the source code.
