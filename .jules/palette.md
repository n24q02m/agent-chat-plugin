...
## 2024-08-01 - Interactive Stdin Prompt
**Learning:** When a CLI reads from stdin without a prompt (e.g., when a user omits body arguments but runs interactively), the process appears to hang, leading to a poor user experience.
**Action:** Use `sys.stdin.isatty()` to detect interactive use and print a helpful prompt to `stderr` (e.g., `(reading body from stdin; press Ctrl-D to finish)`), which prevents the appearance of a hanging process without breaking pipes.
