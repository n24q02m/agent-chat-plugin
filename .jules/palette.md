...
## 2024-08-03 - Interactive stdin prompt
**Learning:** When a CLI reads from stdin, users might encounter a hanging process if they run the command interactively without realizing it is waiting for input.
**Action:** When reading from stdin in the CLI, use `sys.stdin.isatty()` to provide a helpful prompt to `stderr` during interactive use, preventing the appearance of a hanging process without breaking pipes.
