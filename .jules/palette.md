...
## 2026-07-28 - Provide prompt for interactive stdin read
**Learning:** When reading from stdin in the CLI via `sys.stdin.read()`, if the user hasn't piped input, the command appears to hang indefinitely with no visual feedback. Using `sys.stdin.isatty()` allows providing a helpful prompt to `stderr` without breaking pipe workflows.
**Action:** When accepting stdin for data input (like message bodies), always check `isatty()` and print instructions to `stderr` (e.g., "(reading body from stdin, press Ctrl+D to finish)") to prevent user confusion during interactive use.
