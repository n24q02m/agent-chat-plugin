...
## 2024-07-25 - Interactive stdin prompts
**Learning:** When a CLI reads from `sys.stdin` and wait for user input without any prompts, it causes a hanging/frozen experience that confuses users.
**Action:** Always check `sys.stdin.isatty()` before reading large blocks from stdin, and provide an OS-aware hint (e.g. `Ctrl-D` vs `Ctrl-Z`) in `sys.stderr` so users know what to do without polluting piped data.
