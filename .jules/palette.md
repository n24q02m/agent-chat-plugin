...
## 2024-05-25 - Improve EOF instructions for stdin
**Learning:** When reading from stdin interactively, users often don't know the correct key combo for EOF, leaving them stuck in a seemingly hanging process. Providing platform-specific instructions (Ctrl-D for Unix, Ctrl-Z + Enter for Windows) significantly improves usability.
**Action:** When a CLI reads from stdin without an explicit EOF marker, always display the exact keyboard shortcuts needed to conclude the input.
