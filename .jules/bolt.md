...
## 2024-05-23 - The string parsing optimization caused a crash
**Learning:** Using `str.isdigit()` followed by `int()` is not safe in Python because `isdigit()` returns `True` for superscript digits (like `²`), but `int()` raises a `ValueError` for them. The original `re.match(r"(\d+)-")` regex did not have this problem and safely returned `None`. This unhandled exception caused the polling loop to crash. Also, the regex is bounded by a short filename and performs fine, making it not worth rewriting.
**Action:** Do not blindly replace `re.match(r"\d+")` with `str.isdigit()` in critical paths without measuring first and understanding Python's specific Unicode behaviors for `isdigit()`. When an optimization is rejected, log the learning and do not repeat the mistake.
