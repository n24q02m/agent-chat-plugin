...
## $(date +%Y-%m-%d) - Replacing message_files() sort with O(N) linear scans
**Learning:** Functions like `max_seq`, `cmd_wait`, and `session_inbox.main` were calling `message_files()`, which loads all files and sorts them by sequence number. For tasks that only need a maximum value, a count, or to filter messages before sorting, this sorting overhead `O(M log M)` is unnecessary and scales poorly with channel size.
**Action:** Prefer using `Path.glob('*.md')` directly for `O(N)` linear scans or deferred sorting to significantly reduce CPU overhead in frequently polled loops.
