...
## 2024-08-01 - Avoid O(N log N) file sorting when polling
**Learning:** Polling loops (`cmd_wait`) and simple property checks (`max_seq`, unread message counts) were calling `message_files(dir)` which globs and then sorts *all* files in a channel (O(N log N)). As message history grows, doing this every 5 seconds burned significant CPU for operations that only need an O(N) scan.
**Action:** When filtering for new items or computing aggregates, prefer `Path.glob('*.md')` combined with an O(N) max or counting operation. Only sort the subset of genuinely new/relevant items, avoiding sorting the entire channel history.
