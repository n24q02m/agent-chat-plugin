...
## 2024-07-26 - O(N log N) overhead in polling paths
**Learning:** `message_files(chan)` loads all messages and sorts them to guarantee chronological order. Calling it in hot paths like `cmd_wait` (a tight loop) or `hooks/session_inbox.py` (which only counts unread) forces an unnecessary O(N log N) sort just to check properties (like max seq or unread count).
**Action:** Use an O(N) generator loop like `chan.glob("*.md")` for simple checks (`max_seq` or unread counting). In tight loops like `cmd_wait`, use an early exit fast-path on `glob` to avoid the sort entirely when no new messages exist.
