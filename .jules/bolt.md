...
## 2024-08-09 - Avoid O(N log N) sorts in hot paths
**Learning:** Functions like `cmd_wait` and `session_inbox` poll or iterate over all files frequently. Using `chat.message_files(chan_dir)` causes the system to eagerly sort all messages by sequence number, resulting in O(N log N) time complexity. For large directories, this scales poorly and wastes CPU.
**Action:** When polling or counting messages, prefer O(N) linear scans using `Path.glob('*.md')` over loading and sorting all files via `message_files()` to avoid unnecessary CPU overhead.
