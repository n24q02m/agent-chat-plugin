...
`state` renders the goal/topic, decisions, open tasks, blockers, owners/leases,
path locks and verification evidence from local source records. `compact`
writes `<root>/<channel>/state.md` atomically and by default posts a
`state.compacted` audit event. These are deterministic operations, not LLM
summarization. The summary is derived, never authoritative. Compaction does
not delete or truncate messages, task records, active claims, path locks or
read cursors. State failures exit nonzero with stable `STATE_*` codes.

`--reply <seq>` threads a message to an earlier one. `python chat.py <cmd> --help` for all flags.

In Claude Code, `/agent-chat` supplies the read/reply workflow using `$AGENT_CHAT_NAME`.
The optional SessionStart, UserPromptSubmit and Stop hooks only notify; they do
not run the workflow, block a turn or wake peers. Missing identity is silent at
prompt/stop; session start warns when channels exist. Malformed configured
channel names do not suppress valid inboxes. Verify lifecycle/output behavior
in each host rather than treating CLI installation as hook registration.
